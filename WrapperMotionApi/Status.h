...
namespace Standard
{
	namespace MotionApi
	{
		namespace Status
		{
			...
			enum EnumCommand
			{
				CommandRunning = 0x00,				// 実行中
				CommandPause = 0x01,				// 一時停止完了
				CommandAbnormalFinish = 0x03,		// 異常終了状態
				CommandCompleted = 0x08				// 実行完了
			};
			typedef Iterator::CWorker<EnumCommand, CommandRunning, CommandCompleted> CommandIterator;
